...
class MXSelectStock:
    ...
    def extract_data(result: Dict[str, Any]) -> Tuple[List[Dict[str, str]], str, Optional[str]]:
        ...
        # 优先使用全量数据 dataList
        all_results = inner_data.get("allResults") or {}
        ar_result = all_results.get("result") or {}
        data_list = ar_result.get("dataList") or []
        columns = ar_result.get("columns") or []
        ...
        partial_results = inner_data.get("partialResults") or ""
        if partial_results:
